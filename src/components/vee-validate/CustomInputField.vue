<template>
    ...
    <Field v-slot="{ componentField, errorMessage, }" :name>
        ...
        <div class="flex gap-3 w-full  relative">
            <Input v-bind="componentField" :placeholder
                :class="errorMessage ? ' border-error-light-2 focus:ring-transparent' : 'border'"
                class=" shadow appearance-none   rounded-lg py-2  pr-10 text-neutral-20 placeholder:text-neutral-40 leading-tight focus:outline-none focus:ring  h-[44px] " />
                <span class="absolute bottom-1 right-0 p-2">
