<template>
  <div class="bg-white rounded-sm border p-1 relative h-[236px] overflow-hidden">
    ...
    <!-- Nom du produit -->
    <h3 class="font-bold text-xs md:text-sm text-center mb-4 line-clamp-2">
      {{ product.product.name }} {{ toLowercaseFirstLetterOfEachWord(product.label) }}
    </h3>
    ...
    <div class="ml-5">
      <img
        ...
        :alt="`Image de ${product.product.name}`"
    ...
    <section v-if="!product.status" class="pointer-events-noner absolute inset-0">
      ...
      <!-- Bandeau diagonal -->
      <div
        class="absolute top-10 -right-24 w-80 bg-red-600 text-white text-center py-3 font-bold text-xs tracking-wider transform rotate-45 shadow-lg"
      >
        RUPTURE DE STOCK
      </div>
    </section>
...
const isDisabled = computed(
  () =>
    casierProductsData.products.length === 24 &&
    casierProductsData.products.length > 0 &&
    casierProductsData.products[0].id == product.id,
)
