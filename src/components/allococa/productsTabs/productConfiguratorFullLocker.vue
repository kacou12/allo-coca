<template>
  <div class="bg-white rounded-lg lg:shadow-lg lg:p-6 py-6 mx-auto h-full">
    ...
    <section class="h-full flex flex-col">
      ...
      >
        <ProductFullLockerCard
          :casier-products-data="casierProducts"
          :key="product.id"
          v-for="product in products"
          :product="product"
          @full-increment="updateProductQuantity"
        />
      </div>
...
const totalBottles = computed(
  () => casierProducts.products.reduce((sum, p) => sum + p.quantity, 0) * desiredLockers.value,
)
const totalLockers = computed(() => Math.floor(totalBottles.value / CASIER_CAPACITY))
const subTotal = computed(
  () =>
    casierProducts.products.reduce((sum, p) => sum + p.quantity * p.unit_price, 0) *
    desiredLockers.value,
)
