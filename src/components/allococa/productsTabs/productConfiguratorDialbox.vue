<template>
  <div class="bg-white rounded-md lg:shadow-lg lg:p-6 py-6 mx-auto h-full">
    ...
    <section class="h-full flex flex-col">
      <div
        class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-2 lg:gap-6 gap-2 mb-8 flex-1 overflow-y-scroll"
      >
        <ProductLockerCard
          :casier-products-data="casierProducts"
          :key="product.id"
        ...
        <div class="flex items-center justify-between w-full">
          ...
          <div>
            <input
              id="locker-count"
              type="number"
              @change="emit('update:casierQuantity', desiredLockers)"
              v-model.number="desiredLockers"
              min="1"
      ...
      <article class="space-y-4">
        ...
        <button
          class="flex items-center justify-center w-full px-4 text-sm py-3 rounded-[90px] transition-colors duration-200 disabled:text-[#888888] disabled:bg-[#F6F6F6] disabled:cursor-not-allowed bg-red-600 text-white"
        ...
        <!-- <p class="text-xs text-gray-500 mt-2">
          ...
          Constituer au moins un casier ou minimum 10 000 FCFA
        </p> -->
        <div class="h-4"></div>
      </article>
    </section>
...
import type {
  ...
  Product,
  ProductResponse,
} from 'src/services/locker-products/locker-products-type'
...
const { defaultProducts, casierProducts } = defineProps({
  casierProducts: {
...
const emit = defineEmits<{
  (e: 'increase:quantity', product: ProductResponse): void
  (e: 'decrease:quantity', product: ProductResponse): void
...
onBeforeMount(() => {
  products.value = cloneDeep(
    defaultProducts.map((product) => ({
      ...product,
      quantity: getDefaultProductQuantity(product.id),
    })),
  )
})
...
const totalBottles = computed(
  () => products.value.reduce((sum, p) => sum + p.quantity, 0) * desiredLockers.value,
)
const totalLockers = computed(() => Math.floor(totalBottles.value / CASIER_CAPACITY))
const subTotal = computed(
  () =>
    products.value.reduce((sum, p) => sum + p.quantity * p.unit_price, 0) * desiredLockers.value,
)
const maxBottles = computed(() => desiredLockers.value * CASIER_CAPACITY)

const canAddToBasket = computed(
  () => totalBottles.value >= CASIER_CAPACITY && subTotal.value >= MIN_ORDER_AMOUNT,
)
