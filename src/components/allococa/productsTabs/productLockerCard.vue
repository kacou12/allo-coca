<template>
  <div class="bg-white rounded-sm border p-1 h-[236px] relative overflow-hidden">
    ...
    >
      ...
      <span class="text-sm md:text-lg font-semibold w-6 text-center text-gray-800">{{
        product.quantity
      }}</span>

      <button
        @click="increment"
        :disabled="product.quantity == 24 || casierProductsData.products.length == 24"
        :class="[
          product.quantity == 24 || casierProductsData.products.length == 24
            ? 'bg-gray-100 text-gray-300 cursor-not-allowed'
