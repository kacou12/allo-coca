<template>
  <Sheet v-model:open="open">
    <SheetTrigger as-child>
      <section
        :class="[isScrolled ? 'bg-primary-50 border-primary-50' : 'border-white']"
    ...
    (data) => {
      if (has_own_lockers.value) {
        return (
          data.lockerCount &&
          data.lockerCount >= 1 &&
          data.lockerCount <= casierQuantityLength.value
        )
      }
      return true
    },
    {
      message: 'Veuillez sélectionner le nombre valide de casiers que vous souhaitez consigner.',
      path: ['lockerCount'],
    },
  )
