    ...
    <div class="px-4 bg-white">
        ...
        <main class="space-y-5 xl:w-[75%] font-medium text-sm my-8 ">
            <section class="flex flex-col md:flex-row">
                <div class="md:w-[30%]">
                    <p class="flex-1 text-neutral-20">Identifiant (ID)</p>
                </div>
                <div class="flex w-full max-w-[512px] items-center gap-4">
                    <Input v-model="userState.merchant_custom_id" type="text" id="id" placeholder="identifiant"
                        class="w-full" disabled />

                    <CommonClipBoard :data-to-copy="userState.merchant_custom_id!"
                        successText="L'identifiant a bien été copié ">
                    </CommonClipBoard>
...
const userState = reactive({
    ...
    firstname: authUser.user.value?.first_name,  // email de l'utilisateur
    lastname: authUser.user.value?.last_name,  // mot de passe de l'utilisateur
    email: authUser.user.value?.email,
    phone: authUser.user.value?.phone,
    id: authUser.user.value?.id,
    merchant_custom_id: authUser.user.value?.merchant?.merchant_custom_id,
