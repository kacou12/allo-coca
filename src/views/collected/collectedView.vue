  ...
  <div class="px-4 ">
    ...
    <FadeSlideAnimation>
      <section class="grid grid-cols-1 md:grid-cols-3  xl:grid-cols-5 gap-6  " v-if="isFetchedStats">
        <div class="xl:col-span-2 col-span-3 flex  xl:flex-row  flex-col xs:flex-row xs:justify-center gap-6 ">
      ...
      <section class="grid grid-cols-1 md:grid-cols-3  xl:grid-cols-5 gap-6  min-h-[234px]" v-else>
        <!-- SKELETON ICI -->
        <div class="xl:col-span-2 col-span-3 flex xl:flex-row  flex-col xs:flex-row xs:justify-center gap-6 h-full">
          <!-- Skeleton for CommonStatisticCard -->
          <Skeleton class="xl:flex-1 w-full rounded-lg" />
          <div class="xl:flex-1 flex flex-col justify-between gap-5">
    ...
    <section class="my-5">
      ...
      <CommonDataTable :dynamic-width-columns="false" :page-size="limitModel ? parseInt(limitModel) : 10" ref="my-table"
        :default-page="filters.page" :total="transactionsData?.total ?? 0" :columns="collectionAndTransferColumn"
